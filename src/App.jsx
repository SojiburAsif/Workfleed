...
const ReviewSection = () => {
  ...
  return (
    <section className="bg-gradient-to-b from-white via-red-50 to-white py-20 px-6 md:px-12">
        ...
        >
          ...
          {reviews.map((review, idx) => {
            ...
            return (
              <motion.div
                key={idx}
                className="bg-white shadow-xl rounded-2xl p-6 hover:shadow-2xl transition-all duration-300 border-t-4 border-red-500 flex flex-col"
              ...
              >
                ...
                {review.content.length > 100 && (
                  <button
                    ...
                    className="text-red-500 text-xs font-semibold hover:underline mb-2 self-start"
                  >
                    {isExpanded ? 'Show Less' : 'See More'}
                  </button>
                )}
                <p className="text-xs text-gray-500 mt-auto">{review.date} • {review.name}</p>
              </motion.div>
            );
          })}
